        ...
        <div class="goods-content">
            <p class="goods-name">
                <span
                    v-if="goods.tagText"
                    class="goods-tag"
                    :class="{ 'goods-name-tag1': goods.tagStype === 1, 'goods-name-tag2': goods.tagStype === 2 }"
                >{{ goods.tagText }}</span>
                {{ goods.name }}
            </p>
            <div v-for="(sku,index) in goods.sku" :key="index" class="goods-sku-line">
                <div class="goods-sku">
                    ...
                    <img class="sku-arrow" src="@/assets/img/cart/icon-shopcart-arrow-down.png" />
                </div>
            </div>
            <div class="goods-discount-line">
                <div v-if="goods.discount && goods.discount.tip" class="goods-discount">
                    ...
                    <img
                        ...
                        src="@/assets/img/cart/icon-shopcart-arrow-down.png"
                    />
                </div>
                <template v-if="goods.activityNotice && goods.activityNotice.length > 0">
                    <p
                        class="activity-notice"
                        v-for="(noticeText,index) in goods.activityNotice"
                        :key="index"
                    >{{ noticeText }}</p>
                </template>
            </div>
            <div class="price-num-line">
                ...
                <div class="num-wrap">
                    <img
                        class="num-op"
                        v-if="goods.count < 2"
                        src="@/assets/img/cart/icon_cart_minus_dis.png"
                    />
                    <img
                        @click="$emit('decrement', { shopId: shop.id, goodsId: goods.id, value: 1 })"
                        class="num-op"
                        v-else
                        src="@/assets/img/cart/icon_cart_minus_on.png"
                    />
                    <p class="num-text">{{ goods.count }}</p>
                    <img
                        @click="$emit('increment', { shopId: shop.id, goodsId: goods.id, value: 1 })"
                        class="num-op"
                        src="@/assets/img/cart/icon_cart_add_on.png"
                    />
                </div>
            </div>
            <div class="m-action-wrap">
                <div class="m-action">
                    ...
                    <p
                        @click="$emit('remove', { shopId: shop.id, goodsId: goods.id })"
                        class="action-text"
                    >删除</p>
                </div>
            </div>
...
import { Shop, ShopGoods } from '@/common/bean'

import { defineProps } from 'vue'
defineProps<{ goods: ShopGoods, shop: Shop }>()
const priceInt = (price: number) => (price / 100).toFixed(0)
const priceDecimal = (price: number) => {
    const value = (price % 100 / 100)
