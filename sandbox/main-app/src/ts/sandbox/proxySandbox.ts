...
const cachedGlobalsInBrowser = array2TruthyObject(
  globalsInBrowser.concat( ['mockNativeWindowFunction'] ),
);
...
const cachedGlobalObjects = array2TruthyObject(cachedGlobals as []);
...
export default class ProxySandbox implements SandBox {
  ...
  inactive() {
      ...
      console.info(`[qiankun:sandbox] ${this.name} modified global properties restore...`,
        Array.from(this.updatedValueSet.keys())
      );
  ...
  constructor(name: string, globalContext = window, opts?: { speedy: boolean }) {
    ...
    const proxy = new Proxy(fakeWindow, {
      ...
      get: (target: FakeWindow, p: PropertyKey): any => {
        ...
        const value = actualTarget[p as number];
